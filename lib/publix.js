...
exports.parse = function (text, callback) {
  var events = [];
  var match = /<td><DIV>(.*?)<\/DIV><\/td><td><DIV>(.*?)<\/DIV><\/td><td><DIV>(.*?)<\/DIV><\/td><td><DIV>(.*?)<\/DIV><\/td><td class="numberMeHeader"><DIV>(.*?)<\/DIV><\/td><td class="numberMe"><DIV>(.*?)<\/DIV><\/td>/g.exec(text);
  while (match !== null) {
    events.push({
      date: match[1],
      time: match[2]
...
function verify_response(err, res) {
  ...
  if (res.status !== 200) { // || res.text.search(/<title>\s*?Oasis Self Service: Error Page\s*?<\/title>/)) {
